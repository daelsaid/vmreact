...
all_data = pd.read_csv(os.path.join(
    data_dir, 'mturk_vmreact_complete_compilation.csv'))
indexed_df = pd.DataFrame(data=all_data.set_index(
    ['gender_response', 'age_range']))
...
recall_df = pd.DataFrame(data=all_data, columns=[
                         r for r in trial_latency_cols if 'response' in r or 'subject' in r])
...
pd.concat([all_data, new], axis=1).to_csv(os.path.join(
    data_dir, 'mturk_vmreact_complete_compilation_initiation.csv'))
...
for data_file in glob(os.path.join(data_dir, '*raw.csv')):
    ...
    subj_list = data_df.loc[data_df['trialcode']
                            == 'trial8', 'subject'].unique()
    if len(subj_list) > 0:
        data_df.loc[(data_df['trialcode'] == 'trial6') & (
            data_df['subject'].isin(subj_list)), 'trialcode'] = 'listb'
        data_df.loc[(data_df['trialcode'] == 'trial7') & (
            data_df['subject'].isin(subj_list)), 'trialcode'] = 'trial6'
        data_df.loc[(data_df['trialcode'] == 'trial8') & (
            data_df['subject'].isin(subj_list)), 'trialcode'] = 'trial7'
