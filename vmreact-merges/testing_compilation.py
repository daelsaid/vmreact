...
def grader(all_subj_data_csv, data_output_raw_csv, data_output_scored_csv, word_corr, p_r):
    with open(all_subj_data_csv, 'U') as file:
        ...
        all_subj_csv_lines = dict((rows[0], rows[1:])
                                  for rows in input_csv_lines_all_subj)

    subj_listtype = []
    for idx, row in enumerate(all_subj_csv_lines['subject']):
        if 'rey_list' in all_subj_csv_lines['trialcode'][idx]:
            subj_listtype.append(
                [all_subj_csv_lines['subject'][idx], all_subj_csv_lines['trialcode'][idx]])
    ...
    for list_type in sorted(index_number_resp.keys()):
        rey_recall_word_count = collections.Counter(
            index_number_resp[list_type])
        counter_dict[list_type] = rey_recall_word_count

    total_response_for_list = dict()
    for list_type in sorted(index_number_resp.keys()):
        total_response_for_list[list_type] = sorted(
            set(index_number_resp[list_type]))
    ...
    with open(word_corr, 'wb') as csvfile:
        ...
        for word_list in sorted(total_response_for_list.keys()):
            ...
            for word in total_response_for_list[word_list]:
                wordcorrs = [round(SequenceMatcher(None, word, x).ratio(), 3)
                             for x in rey_word_lists[word_list]]
                word_corrs.append(wordcorrs)
                writer.writerow(
                    [word, max(wordcorrs), rey_word_lists[word_list][wordcorrs.index(max(wordcorrs))]])
    csvfile.close()
    ...
    with open(data_output_raw_csv, 'wb') as csvfile:
        ...
        for response in full_raw_data_responses:
            subj = response[0]
            list_to_use = [subj_id_list[x][1] for x in range(
                len(subj_id_list)) if subj_id_list[x][0] == subj][0]
            ...
            if 'listb' in response[1]:
                if response[2] in rey_word_lists[list_b]:
                    ...
                    if any(n > 0.8 for n in
                        ...
                        response.append(0)
                new_row = response[0], list_b, response[1].split(
                    '_')[0], response[2], response[3]
            else:
                if response[2] in rey_word_lists[list_to_use]:
                    ...
                    if any(n > 0.8 for n in
                        ...
                        response.append(0)
                new_row = response[0], list_to_use, response[1].split(
                    '_')[0], response[2], response[3]
...
def demo_and_summary(all_subj_data_csv, demographic_data, final_summary_csv, frequency_count, subj_age_agerange_gender,
    ...
    with open(demographic_data, 'U') as file:
        ...
        demographic_data = dict((rows[0], rows[1:])
                                for rows in (input_demo_sr_q_csv))
    ...
    with open(final_summary_csv, 'U') as file:
        ...
        rey_summary = dict((rows[0], rows[1:])
                           for rows in (final_summary_lines))
    ...
    with open(subj_age_agerange_gender, 'wb') as csvfile:
        ...
        for subj in sorted(demo_subj_age_gender):
            ...
            gender = subj[1]
            subj_id_raw = [
                val for val in raw_data_responses if val[0] == subj[0]]
            for vals in key_val:
                age_vals = vals[1]
                age_vals = str(age_vals)
                if age_vals == ages:
                    complete_list = subj[0] + ',' + \
                        gender + "," + age_vals + "," + vals[0]
...
def composite_scores(input_csv, output_csv):
    ...
    composite_scores['corrected_total_learning'] = composite_scores['total_learning'].subtract(
        tmp['test'])

    composite_scores['learning_rate'] = df_trials['trial5'].subtract(
        df_trials['trial1'], axis='rows')
    composite_scores['proactive_interference'] = df_trials['trial1'].subtract(
        scored_data['listb'], axis='rows')
    composite_scores['retroactive_interference'] = df_trials['trial5'].subtract(
        df_trials['trial6'], axis='rows')

    composite_scores['forgetting_and_retention'] = df_trials['trial5'].subtract(
        df_trials['trial7'], axis='rows')

    composite_scores_transposed = composite_scores.transpose()

    composite_scores_transposed.to_csv(
        output_csv, header=True, index=['measure', 'score'])
    composite_scores.to_csv(output_csv, header=True,
                            index=['measure', 'score'])
...
def standardize_and_rename_scored_csvs(output_csv_location, data_path, mturk_data_set):
    if mturk_data_set == 0:
        for data in glob(os.path.join(raw_data_csvs, '*.csv')):
            print data
            main_dir = data.split('/')[-2]
            filename = data.split('/')[-1]
            outdir == os.path.basename()

    elif mturk_data_set == 1:
        for raw in glob('/Users/cdla/Desktop/scratch/vmreact/1_rawdata/*/*raw.csv'):
            ...
            prefix = 'mturk_' + \
                os.path.basename(os.path.dirname(raw_data)).split('_')[1] + '_'
            grader(raw_data, os.path.join(output_csv_location, prefix + 'parsed_raw_data.csv'),
                   os.path.join(output_csv_location,
                                prefix + 'scored_data.csv'),
            ...
            copy(summary_data, os.path.join(
                output_csv_location, prefix + 'summary.csv'))
...
def rename_with_subj(subj_dir):
    files = glob(os.path.join(subj_dir, 'out', '*.csv'))
    ...
    for file in range(0, len(files)):
        ...
        new = os.path.join(path, 'best_' + prefix_id.split('_')
                           [1] + '_' + prefix_id.split('_')[2] + '_' + fname)
        try:
            os.rename(old, new)
        except OSError:
...
main_dir = '/Users/lillyel-said/Desktop/inquisit_renamed/'
# defining all dataframes from scored data


def compiled_columns(scored_dir):
    demo_cols = []
    clin_raw_cols = []
    ...
    if mturk == 1:
        ...
        for batch in range(1, 9):
            ...
            age_range_gender_csv = os.path.join(
                scored_dir, prefix + batch + '_age_range_gender.csv')
            demo = os.path.join(scored_dir, prefix + batch + '_demo.csv')
            clin_raw = os.path.join(scored_dir, prefix + batch + '_end.csv')
            summ = os.path.join(scored_dir, prefix + batch + '_summary.csv')
            scored = os.path.join(scored_dir, prefix +
                                  batch + '_scored_data.csv')
            composite = os.path.join(
                scored_dir, prefix + batch + '_composite_scores.csv')
...
def compiled_columns(scored_dir):
    ...
    # for dataset in glob(os.path.join(scored_dir,'*.csv')):
    # 		subj=dataset.split('/')[-3]
    # 		prefix_id='best_'+subj.split('_')[1]+'_'+subj.split('_')[2]
    # 		filename=prefix_id+'_'+dataset.split('/')[-1]
    # 		csv_path=os.path.join(scored_dir,subj,'csv')
    # 		age_range_gender_csv = os.path.join(scored_dir, prefix_id+ '_age_range_gender.csv')
...
scored_data = os.path.join(args.output_csv_location,
