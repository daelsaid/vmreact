...
# data_dir='/Volumes/wd_daelsaid/inquisit/temp/to_score/best_1002_tp2_inquisit/csv'
# output_dir='/Volumes/wd_daelsaid/inquisit/temp/to_score/best_1002_tp2_inquisit/out'
...
def gen_vmreact_latencies(data_dir,output_dir):
    ...
    for file in glob(os.path.join(data_dir,'*_raw.csv')):
        try:
            ...
            datadf.loc[datadf['response']==' ','trialcode'] = 'trial_confirmation'

            for trial_name in ['trial1','trial2','trial3','trial4','trial5','trial6','trial7','trial8','listb']:
                datadf.loc[data_df['trialcode'].str.contains(trial_name),'trialcode']=trial

            datadf.rename(columns={'latency':'values.response_latency'},inplace=True)
    ...
    for data_file in glob(os.path.join(data_dir,'*_raw.csv')):
        try:
            ...
            data_df.loc[data_df['response']==' ','trialcode'].any() == 'trial_confirmation'
    ...
    final_csv_no_duplicates.to_csv(os.path.join(output_dir,'best_vmreact_latencies_summary.csv'),index=False,header=False)
