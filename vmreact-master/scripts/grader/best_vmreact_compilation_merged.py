...
def restructure_and_regrade_all_data(scored_dir):
    ...
    # for raw in glob(os.path.join(os.path.dirname(scored_dir))):
        ...
        sum_df = pd.read_csv(os.path.join(csv_dir,summary), dtype=str).rename(index=str, columns={'script.subjectid': 'subject'})
        ...
        df = primacy_df.merge(recency_df, on='subj_id', left_index=True,how='left',suffixes=('_primacy','_recency')).rename(columns={'subj_id': 'subject'})
        ...
        vmreact_df=demo_df.merge(vmreact_df.astype(str),left_on='subject',right_on='subj_id_x',how='left')
        ...
        vmreact_df.merge(sum_df, left_on='subject', right_on='subject', how='left').to_csv(os.path.join(out_dir,'_'.join(path)+'_vmreact_compiled_'+date+'.csv'))
