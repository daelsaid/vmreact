...
def ant_extraction_and_latencies(raw_path,output_dir,template_dir,cleaned_dir):
        ...
        try:
            ...
            for subj,subj_df in ant_r.groupby(['subject']):
                ...
                try:
                    ...
                    subj_df_compiled.to_csv(os.path.join(output_dir,subj_id_prefix+'_parsed_raw_antr_data_'+date+'.csv'))
                except:
                    subj_df_compiled.to_csv(os.path.join(output_dir,subj_id_prefix+'_parsed_raw_antr_data_'+date+'.csv'))
        except:
            continue
    ...
    for batch_csv in sorted(glob(os.path.join(output_dir,'*antr*.csv'))):
        try:
            ...
            data.to_csv(os.path.join(cleaned_dir,subj_id_prefix+'_extracted_antr_raw_data_'+date+'.csv'),index=False)
