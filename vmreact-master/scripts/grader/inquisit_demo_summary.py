...
def demo_and_summary(all_subj_data_csv, demographic_data, final_summary_csv, frequency_count, subj_age_agerange_gender, sr_responses, summary_ant_scores):
    ...
    for subject in sorted(set(all_subj_csv_lines['subject'])):
        ...
        subj_id_list_combined = [demographic_data['subject'][x] for x in range(
            len(demographic_data['subject'])) if demographic_data['subject'][x] == subject]
    ...
    subject_summary_sr_responses = [[rey_summary['script.subjectid'][x], rey_summary['expressions.gad_7_total'][x], rey_summary['expressions.phq_total'][x], rey_summary['expressions.pcl_4_total']
                                     [x], rey_summary['expressions.pcl_total_hybridscore_corrected'][x]] for x in range(len(rey_summary['script.subjectid'])) if rey_summary['values.end_survey_completed'][x] == '1']

    subject_summary_ant_scores = [[rey_summary['script.subjectid'][x], rey_summary['expressions.overallpercentcorrect'][x], rey_summary['expressions.meanRT']
                                   [x], rey_summary['expressions.stdRT'][x]] for x in range(len(rey_summary['script.subjectid'])) if rey_summary['values.end_survey_completed'][x] == '1']
